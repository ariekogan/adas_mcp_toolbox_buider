...
export function generateMCPServer(toolbox, { skillYaml } = {}) {
  ...
  // Embed skill definition as JSON for ADAS Core discovery via get_skill_definition
  // JSON avoids the YAML escaping issues (persona fields with \n, ", * break YAML
  // when double-escaped for Python triple-quoted strings)
  const skillJson = JSON.stringify(toolbox, null, 2);
  const escapedJson = skillJson.replace(/\\/g, '\\\\').replace(/"""/g, '\\"\\"\\"');
  const escapedName = escapeString(toolbox.name || 'MCP Server');
...
# ── Skill Definition JSON (embedded for ADAS Core get_skill_definition) ──
