...
export function generateMCPServer(toolbox, { skillYaml } = {}) {
  ...
  // Embed skill definition YAML for ADAS Core discovery via get_skill_definition
  // If not provided, generate it from the toolbox
  const yamlContent = skillYaml || generateSkillYaml(toolbox);
...
# ── Embedded Skill Definition (for ADAS Core discovery) ──────────────
...
def get_skill_definition() -> dict:
    ...
    Returns the full skill configuration including:
    ...
    return {
        "format": "yaml",
        "content": SKILL_DEFINITION_YAML,
        "skill_name": "${escapeString(toolbox.name || name)}",
        "version": "${version}"
    }
...
def health_check() -> dict:
    ...
    return {"status": "healthy", "skill": "${escapeString(toolbox.name || name)}", "version": "${version}"}
...
export function generateSkillYaml(skill) {
  ...
  lines.push(...yamlConnectorsBlock(skill.tools));
  lines.push(...yamlUiPluginsBlock(skill.ui_plugins));
  ...
  lines.push(...yamlGrantMappingsBlock(skill.grant_mappings));
  lines.push(...yamlAccessPolicyBlock(skill.access_policy));
  lines.push(...yamlResponseFiltersBlock(skill.response_filters));
  lines.push(...yamlContextPropagationBlock(skill.context_propagation));
...
export function generateExportFiles(toolbox) {
  ...
  const skillYaml = generateSkillYaml(toolbox);
  return [
    { name: "skill.yaml", content: skillYaml },
    { name: "mcp_server.py", content: generateMCPServer(toolbox, { skillYaml }) },
