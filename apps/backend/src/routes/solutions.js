...
router.get('/:id/health', async (req, res, next) => {
  try {
    ...
    //
    // Exempt builtin/auto-generated sources (e.g. `auto-orchestrator`): when
    // `routing_mode === 'auto'`, the Builder synthesizes orchestrator handoffs
    // with `_auto_generated: true` and `from: "auto-orchestrator"`. The
    // orchestrator is a real runtime entity managed by Core — not a skill in
    // solution.skills[]. Flagging it as "not deployed" creates ~N cosmetic
    // warnings (one per auto handoff) that the user can't fix without adding
    // a stub to the solution definition. Skip instead.
    const BUILTIN_HANDOFF_SOURCES = new Set(['auto-orchestrator']);
    for (const h of handoffs) {
      const isAutoGenerated = h._auto_generated === true || BUILTIN_HANDOFF_SOURCES.has(h.from);
      const fromSkill = findSkill(h.from);
      const toSkill = findSkill(h.to);
      if (!isAutoGenerated && (!fromSkill || !fromSkill.deployed)) {
        issues.push({ severity: 'warning', handoff: h.id, message: `Handoff source "${h.from}" not deployed` });
      }
      if (!toSkill || !toSkill.deployed) {
        issues.push({ severity: 'warning', handoff: h.id, message: `Handoff target "${h.to}" not deployed` });
      }
    }

    // Identity check.
    //
    // `actor_types` is only meaningful when the solution has multiple actor
    // categories (e.g. owner / household-member / guest). Single-owner
    // solutions don't need it — there's exactly one implicit actor type.
    // Skip the warning when `identity_mode === 'single-owner'` (or unset)
    // so single-tenant solutions like ada don't accumulate a permanent
    // warning they can't address without stub data.
    const identity = solution.identity || {};
    ...
    const identityDeployed = (identity.actor_types || []).length > 0;
    if (requiresActorTypes && !identityDeployed) {
      issues.push({ severity: 'warning', message: 'No identity actor types defined' });
    }
