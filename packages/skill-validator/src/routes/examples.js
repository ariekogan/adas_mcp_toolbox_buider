...
function buildExampleScriptCacheSkill() {
  const SCRIPT_HINT = [
    "# Solution-author hint. The baker uses this as a starting point and will",
    "# rewrite it if selectors are wrong or steps missing. No failure_class",
    "# required here — the baker adds it automatically on every bake.",
    "",
    "nav = adas_call_tool('web.navigate', {'url': 'https://example.com/account'})",
    "check = adas_call_tool('web.evaluate', {",
    "    'script': \"document.querySelector('.user-badge') ? 'in' : 'out'\"",
    "})",
    "state = str(check.get('result', ''))",
    "adas_output_json({'ok': True, 'logged_in': 'in' in state})",
  ].join("\n");

  return {
    _explanation:
      'Minimal fat-tool skill demonstrating the Level 2 script_cache opt-in. Compared to Level 1, the solution author writes: (a) script_cache:{enabled:true} on the tool, (b) an OPTIONAL script_hint showing what they think the Python should look like. That is all. No persona rules, no Python template in the description, no tool_name awareness. The planner calls site.login({}) directly — the Core synthesizes a dispatcher, the baker writes the Python on first call, caches it, replays on subsequent calls. See Docs/WIP/SCRIPT-LEVEL-JIT-SHORTCUTS.md for design.',
    _how_the_flow_works: [
      '1. Planner sees site.login in the toolbox.',
      '2. Planner calls site.login({}) directly — no run_python_script involved at the planner level.',
      '3. Core sees no runtime fn for site.login but script_cache.enabled:true → synthesizes a dispatcher.',
      '4. Dispatcher calls runPythonScriptCore with tool_name="site.login" + NO code.',
      '5. Cache lookup — MISS on first call. Baker generates Python from tool description + optional script_hint.',
      '6. Baker dry-runs it. Success → cached.',
      '7. Next call hits the cache → replays cached Python, no LLM regen.',
      '8. If the site changes and Python returns failure_class:domain_break → cache invalidated + rebaked next time.',
      '9. If the call fails with failure_class:logical (rate limit, bad creds) → cache PRESERVED, user sees error.',
    ],
    _what_changed_from_level_1: [
      'No Python template in the tool description — the baker generates it.',
      'No persona paragraph about tool_name — the planner does not see run_python_script at all.',
      'No failure_class enforcement in the description — the baker prompt enforces it automatically.',
      'OPTIONAL: script_hint field lets the solution author guide the first bake. The baker treats it as a starting point, not binding.',
    ],
    skill: {
      id: 'example-script-cache',
      name: 'Example: Script Cache Skill (Level 2)',
      description: 'Reference implementation of script-level JIT shortcuts. The planner calls site.login directly — no Python awareness required.',
      version: '1.0.0',
      phase: 'DEPLOYED',
      ...
      problem: {
        ...
        context: 'Browser automation is inherently flaky — CSS classes rotate, elements move, selectors break. Without caching, the LLM regenerates the same Python on every call (wasteful tokens, inconsistent behavior). With script_cache, the working script is cached and only re-generated on actual failure.',
        goals: ['Stable browser automation across DOM drifts', 'Automatic re-baking when the external system changes', 'Preserve cache across user-side failures'],
      },
      role: {
        name: 'Example Automation Agent',
        // Notice how short this is compared to Level 1 — no rules about
        // tool_name, no failure_class documentation, no run_python_script routing.
        persona: [
          'You are a web automation specialist. Use site.login to check login status.',
          '',
          'Mobile chat — 1-2 sentences max. Plain text, no bullet lists.',
        ].join('\n'),
        goals: ['Drive web-based automations'],
      ...
      intents: {
        supported: [
          {
            id: 'check-site-login',
            description: 'Check whether the user is logged in to the target site.',
            examples: ['check my site login', 'am I logged in?'],
            candidate_tools: ['site.login'], // planner calls it DIRECTLY
          },
        ],
      ...
      tools: [
        {
          id: 'tool-site-login',
          name: 'site.login',
          // Plain description — no Python template. The baker generates the
          // implementation from this description alone (plus optional script_hint).
          description: 'Check whether the user is logged into example.com. Navigate to the account page, look for a user-badge marker, return {ok, logged_in}.',
          inputs: [],
          output: { type: 'object', description: '{ ok, logged_in, failure_class }' },
          source: { type: 'custom' },
          security: { classification: 'internal' },
          // Level 2 minimal opt-in.
          script_cache: { enabled: true },
          // Optional: starting-point Python. Baker rewrites it if wrong.
          script_hint: SCRIPT_HINT,
        },
        {
          id: 'tool-browser-wildcard',
          name: 'browser-mcp:*',
          description: 'All browser-mcp tools. The script_cache dispatcher makes these available to baked scripts via the skill\'s allowed_tools.',
          inputs: [],
      ...
      policy: {
        guardrails: {
          never: [],
          always: ['Mobile chat — keep responses 1-2 sentences'],
        },
    ...
    _what_to_verify: [
      ...
      'The tool description is a PLAIN description (no Python template, no failure_class docs).',
      'The persona contains NO rules about tool_name, run_python_script, or failure_class.',
      'Optional: script_hint shows the baker the intended structure. It is NOT binding.',
    ],
  };
}
